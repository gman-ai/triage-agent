...
def test_timeout_failure_emits_span_with_error_type_and_message():
    registry = PlanTemplateRegistry()
    plan = registry.build_plan("ransomware", "P1")
    sources = build_default_registry()
    bundle = run_fanout(
    ...
    assert span["error_type"] == "RetrievalTimeoutError"
    assert "timed out" in (span["error_message"] or "").lower()
    assert span["retrieved_count"] == 0
    assert "threat_intel" in bundle.enrichments_failed


def test_upstream_5xx_failure_emits_upstream_error_outcome():
    registry = PlanTemplateRegistry()
    plan = registry.build_plan("ransomware", "P1")
    sources = build_default_registry()
    bundle = run_fanout(
    ...
    assert failed[0]["error_type"] == "RetrievalUpstreamError"
...
def test_malformed_failure_emits_malformed_outcome():
    ...
    assert failed[0]["error_type"] == "MalformedRetrievalError"
...
def test_tier_policy_rejection_surfaces_in_spans():
    ...
    outcome=rejected and error_type=TierPolicyExcluded so SRE sees a
    ...
    assert rejection_spans[0]["error_type"] == "TierPolicyExcluded"
    assert "tier_preference" in (rejection_spans[0]["error_message"] or "")
